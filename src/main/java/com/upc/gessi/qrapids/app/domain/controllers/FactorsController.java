...
public class FactorsController {
     ...
     * Iterable<QFCategory> factorCategoriesIterable =
     * factorCategoryRepository.findAll();
    ...
    public List<QFCategory> getFactorCategories(String name) {
        ...
        if (name != null)
            factorCategoryIterable = factorCategoryRepository.findAllByName(name);
        else
            factorCategoryIterable = factorCategoryRepository.findAll();
        factorCategoryIterable.forEach(factorCategoryList::add);
        return factorCategoryList;
    }
    // Old getFactorCategories
    /*
     * public void getFactorCategories(List<Map<String, String>> categories) throws
     * CategoriesException {
    ...
    public void updateFactorCategory(List<Map<String, String>> categories, String name, String patternGroup)
            throws CategoriesException {

        if (checkIfCategoriesHasRepeats(categories))
            throw new CategoriesException(Messages.CATEGORIES_HAVE_REPEATS);
    ...
    public void newFactorCategories(List<Map<String, String>> categories, String name, String patternGroup)
            throws CategoriesException {

        boolean exists = CheckIfNameExists(name);
        if (exists)
            throw new CategoriesException(Messages.CATEGORY_ALREADY_EXISTS);
    ...
    public Factor findFactorByExternalIdAndProjectId(String externalId, Long prjId)
            throws QualityFactorNotFoundException {
    ...
    public void importFactorsAndUpdateDatabase()
            throws IOException, CategoriesException, ProjectNotFoundException, MetricNotFoundException {
        ...
        for (String prj : projects) {
            ...
            List<DTODetailedFactorEvaluation> factorsWithMetrics = getAllFactorsWithMetricsCurrentEvaluation(prj, null,
                    false);
        ...
        for (DTOFactorEvaluation factor : factors) {
            ...
            if (factorsSaved == null) {
                try {
                    ...
                    // Log the error but continue with the next factor
                    org.slf4j.LoggerFactory.getLogger(FactorsController.class).error(
                            "Error importing factor " + factor.getId() + ": Metric not found. Skipping this factor.",
                            e);
                } catch (Exception e) {
    ...
    private Factor saveImportedQualityFactor(String id, String name, String description, List<String> qualityMetrics,
            Project project) throws MetricNotFoundException {
    ...
    public Factor getQualityFactorByExternalIdAndProjectId(String qualityFactorExternalId, Long projectId)
            throws QualityFactorNotFoundException {
        Optional<Factor> qualityFactorOptional = qualityFactorRepository
                .findByExternalIdAndProject_Id(qualityFactorExternalId, projectId);
        if (qualityFactorOptional.isPresent()) {
            return qualityFactorOptional.get();
        } else {
            throw new QualityFactorNotFoundException(qualityFactorExternalId);
    ...
    public Factor saveQualityFactorWithCategory(String name, String description, String threshold,
            List<String> qualityMetrics, String type, String category, Project project) throws MetricNotFoundException {
        Factor qualityFactor;
        // create Quality Factor minim (without quality factors and weighted)
    ...
    private boolean assignQualityMetricsToQualityFactor(List<String> qualityMetrics, Factor qualityFactor)
    ...
    public Factor editQualityFactor(Long factorId, String name, String description, String threshold,
            List<String> qualityMetrics, String type)
            throws QualityFactorNotFoundException, QualityFactorMetricsNotFoundException, MetricNotFoundException {
    ...
    public Factor editQualityFactorWithCategory(Long factorId, String name, String description, String threshold,
            List<String> qualityMetrics, String type, String category)
            throws QualityFactorNotFoundException, QualityFactorMetricsNotFoundException, MetricNotFoundException {
        Factor factor = getQualityFactorById(factorId);
        factor.setName(name);
        factor.setDescription(description);
    ...
    private boolean reassignQualityMetricsToQualityFactor(List<String> qualityMetrics, Factor factor)
            throws QualityFactorMetricsNotFoundException, MetricNotFoundException {
    ...
    public boolean assessQualityFactors(String projectExternalId, LocalDate dateFrom)
    ...
    private boolean assessDateQualityFactors(String projectExternalId, LocalDate dateFrom)
            throws IOException, CategoriesException, ProjectNotFoundException {
        boolean correct = true;
    ...
    private boolean assessDateProjectQualityFactors(String project, LocalDate evaluationDate)
        ...
        // If we receive an evaluationData is because we are recomputing historical
        // data.
        ...
        if (evaluationDate == null) {
            ...
            metricList = metricsController.getAllMetricsHistoricalEvaluation(project, null, evaluationDate,
                    evaluationDate);
            // when there is no historic metrics values for specified day --> take the last
            // assessment
    ...
    private boolean assessProjectQualityFactors(LocalDate evaluationDate, String projectExternalId,
            MetricEvaluation metricEvaluationQMA) throws IOException {
        ...
        // 1.- We need to remove old data from metric evaluations in the quality_factor
        // relationship attribute
        // metricEvaluationQMA.clearQualityFactorsRelations(evaluationDate);

        // 2.- We will compute the evaluation values for the QFs, adding the
        // corresponding relations to the metrics
        ...
        // 3. When all the quality factors is calculated, we need to update the metrics
    ...
    private boolean assessQualityFactor(LocalDate evaluationDate, String project, Factor qualityFactor,
            MetricEvaluation metricEvaluationQMA) throws IOException {
        boolean correct = true;
        ...
        List<String> missingMetrics = new ArrayList<>(); // List of metrics without assessment ---> QF assessment
                                                         // incomplete
        ...
        metricsMismatch = buildMetricsInfoAndCalculateMismatch(evaluationDate, project, qualityFactor,
                metricEvaluationQMA, listMetricsAssessmentValues, qfMetrics, metricList, missingMetrics,
                metricsMismatch, mapQFMetrics);

        String assessmentValueOrLabel = "";
        try {
            assessmentValueOrLabel = assessQualityFactors(evaluationDate, project, qualityFactor,
                    listMetricsAssessmentValues, qfMetrics, missingMetrics, metricsMismatch, assessmentValueOrLabel);
        ...
        if (correct && !assessmentValueOrLabel.isEmpty()) {
            correct = buildAndSaveMetricQFRelation(evaluationDate, project, qualityFactor, metricList,
                    assessmentValueOrLabel);
        }

        return correct;
    }

    private long buildMetricsInfoAndCalculateMismatch(LocalDate evaluationDate, String project, Factor qualityFactor,
            MetricEvaluation metricEvaluationQMA, List<Float> listMetricsAssessmentValues, List<String> qfMetrics,
            List<DTOMetricEvaluation> metricList, List<String> missingMetrics, long metricsMismatch,
            Map<String, String> mapQFMetrics) throws IOException {
        int index;
        boolean metricFound;
        ...
        for (String mID : qfMetrics) {
            ...
            // We need to find the assessment of the metric in the QF definition, in case
            ...
            while (!metricFound && index < metricEvaluationQMA.getMetrics().size()) {
                ...
                if (metric.getId().equals(mID)) {
                    ...
                    // metric.addQualityFactors(evaluation.Factor.getHardID("",
                    // qualityFactor.getExternalId(), evaluationDate));
    ...
    private boolean buildAndSaveMetricQFRelation(LocalDate evaluationDate, String project, Factor qualityFactor,
            List<DTOMetricEvaluation> metricList, String assessmentValueOrLabel) throws IOException {
        boolean correct;
        ...
        for (DTOMetricEvaluation dtoMetricEvaluation : metricList) {
            ...
            if (weight == -1f) {
                values.add(dtoMetricEvaluation.getValue() * 1f / metricList.size()); // value for representation
                                                                                     // (average)
            } else {
        ...
        correct = saveMetricQFRelation(project, metricsIds, qualityFactor.getExternalId(), evaluationDate, weights,
                values, labels, assessmentValueOrLabel);
        return correct;
    }

    private boolean saveMetricQFRelation(String prj, List<String> metricsIds, String qf, LocalDate evaluationDate,
            List<Float> weights, List<Float> metricValues, List<String> metricsLabels, String qfValueOrLabel)
            throws IOException {
        return qmaRelations.setQualityFactorMetricRelation(prj, metricsIds, qf, evaluationDate, weights, metricValues,
                metricsLabels, qfValueOrLabel);
    }

    private String assessQualityFactors(LocalDate evaluationDate, String project, Factor qualityFactor,
            List<Float> listMetricsAssessmentValues, List<String> qfMetrics, List<String> missingMetrics,
            long metricsMismatch, String assessmentValueOrLabel)
            throws IOException, AssessmentErrorException, CategoriesException {
        if (!listMetricsAssessmentValues.isEmpty()) {
            ...
            for (int j = 0; j < listMetricsAssessmentValues.size(); j++) {
                ...
                if (weighted)
                    metricInfo += "weight: " + weights.get(j).intValue() + "%);";
                else
                    metricInfo += "no weighted);";
                info += metricInfo;
            }
            if (weighted) {
                info += " }, formula: weighted average, value: " + value + ", category: "
                        + qualityFactor.getCategoryName();
            } else {
            ...
            for (StrategicIndicatorQualityFactors siqf : strategicIndicatorQualityFactorsRepository
                    .findByQuality_factor(qualityFactor)) {
                // DTOStrategicIndicatorEvaluation si =
                // strategicIndicatorsController.getSingleStrategicIndicatorsCurrentEvaluation(siqf.getStrategic_indicator().getExternalId(),
                // project);
                // indicators.add(StrategicIndicator.getHardID(project, si.getId(),
                // si.getDate()));
    ...
    public boolean assessQualityFactor(String name, String prj) throws IOException, ProjectNotFoundException {
        ...
        // 1.- We need to remove old data from metric evaluations in the quality_factors
        // relationship attribute
    ...
    public List<DTOFactorEvaluation> getAllFactorsEvaluation(String projectExternalId, String profile, boolean filterDB)
            throws IOException {
        return qmaQualityFactors.getAllFactors(projectExternalId, profile, filterDB);
    }

    public List<DTODetailedFactorEvaluation> getAllFactorsWithMetricsCurrentEvaluation(String projectExternalId,
            String profile, boolean filterDB) throws IOException, ProjectNotFoundException {
        return qmaQualityFactors.CurrentEvaluation(null, projectExternalId, profile, filterDB);
    }

    public List<DTODetailedFactorEvaluation> getFactorsWithMetricsForOneStrategicIndicatorCurrentEvaluation(
            String strategicIndicatorId, String projectExternalId) throws IOException, ProjectNotFoundException {
        // we are looking for factors of one SI -> profile is already checked by SI
        // Controller & we need to filterDB (if factor is involved in SI, it can't be
        // deleted)
        return qmaQualityFactors.CurrentEvaluation(strategicIndicatorId, projectExternalId, null, true);
    }

    public List<DTOFactorEvaluation> getAllFactorsHistoricalEvaluation(String projectExternalId, String profile,
    ...
    public List<DTODetailedFactorEvaluation> getAllFactorsWithMetricsHistoricalEvaluation(String projectExternalId,
            String profile, LocalDate dateFrom, LocalDate dateTo) throws IOException, ProjectNotFoundException {
        return qmaQualityFactors.HistoricalData(null, dateFrom, dateTo, projectExternalId, profile);
    }

    public List<DTODetailedFactorEvaluation> getFactorsWithMetricsForOneStrategicIndicatorHistoricalEvaluation(
            String strategicIndicatorId, String projectExternalId, LocalDate dateFrom, LocalDate dateTo)
            throws IOException, ProjectNotFoundException {
        // we are looking for factors of one SI -> profile is already checked by SI
        // Controller
    ...
    public String getFactorLabelFromNameAndValue(String nameCategory, Float val_metric) {
        ...
        // factorCategoryRepository.findAllByName(nameCategory); TODO: no funciona por
        // lo que he cambiado la implementacion
    ...
    public String getCategoryFromRationale(String rationale) {
        String category = rationale.substring(rationale.indexOf("category") + 10, rationale.length()); // +9 porque te
                                                                                                       // da el indice
                                                                                                       // de c: hay que
                                                                                                       // sumarle toda
                                                                                                       // la palabra y
    ...
    public List<Factor> getQualityFactorsByProjectAndProfile(String prjExternalId, String profileId)
        ...
        if ((profileId != null) && (!profileId.equals("null"))) { // if profile not null
            ...
            if (profile.getAllSIByProject(project)) { // if allSI true --> return all factors
                ...
                List<ProfileProjectStrategicIndicators> ppsiList = profileProjectStrategicIndicatorsRepository
                        .findByProfileAndProject(profile, project);
                List<Factor> result = new ArrayList<>();
                for (ProfileProjectStrategicIndicators ppsi : ppsiList) {
                    List<StrategicIndicatorQualityFactors> siqfList = ppsi.getStrategicIndicator()
                            .getStrategicIndicatorQualityFactorsList();
