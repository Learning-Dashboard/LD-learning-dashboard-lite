...
public class AlertsControllerTest {
    ...
    public void createAlert()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
    ...
    @Test
    public void shouldCreateMetricAlertWithCategoryDowngrade() throws IOException, MetricNotFoundException,
        ...
        // creating a previous evaluation that will have a higher value, so it will be
        // in a different level and raise an alert
        ...
        when(qmaMetrics.SingleHistoricalData(eq(metric.getExternalId()), any(), any(), eq(project.getExternalId()),
                any())).thenReturn(previousEvals);
    ...
    @Test
    public void shouldCreateMetricAlertWithCategoryUpgrade() throws IOException, MetricNotFoundException,
            QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
        ...
        // creating a previous evaluation that will have a lower value, so it will be in
        // a different level and raise an alert
        ...
        DTOMetricEvaluation previousEval = new DTOMetricEvaluation(metricId, metricName, metricDescription, null, null,
    ...
    @Test
    public void shouldCreateMetricAlertWithThresholdTrespassed() throws IOException, MetricNotFoundException,
            QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
        ...
        DTOMetricEvaluation metricEval = domainObjectsBuilder.buildDTOMetric();
        float value = 0.3f; // threshold is 0.5f so the alert should be created at least for the threshold
                            // trespassed
    ...
    @Test
    public void shouldNotCreateMetricAlertBecauseTodayExactAlertHasBeenCreated() throws IOException,
        ...
        when(alertRepository
                .findAlertByProjectIdAndAffectedIdAndAffectedTypeAndTypeAndDateGreaterThanEqualAndDateLessThan(
                        eq(projectId), eq(metric.getExternalId()), eq("metric"),
                        eq(AlertType.CATEGORY_DOWNGRADE), any(Date.class), any(Date.class)))
                .thenReturn(alert);
    ...
    @Test
    public void shouldCreateMetricAlertBecauseTodayExactAlertHasBeenCreatedButDifferent() throws IOException,
            MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    public void shouldCreateFactorAlertWithCategoryDowngrade() throws IOException, ProjectNotFoundException,
        ...
        when(qmaFactors.HistoricalData(eq(factor.getExternalId()), any(), any(), eq(project.getExternalId()), any()))
                .thenReturn(previousEvals);
    ...
    public void shouldCreateFactorAlertWithCategoryUpgrade() throws IOException, ProjectNotFoundException,
        ...
        DTOMetricEvaluation dtoMetricEvaluation = new DTOMetricEvaluation(metricId, metricName, metricDescription, null,
                null, metricRationale, qualityFactors, evaluationDate, metricValue);
        List<DTOMetricEvaluation> dtoMetricEvaluationList = new ArrayList<>();
        dtoMetricEvaluationList.add(dtoMetricEvaluation);
        DTODetailedFactorEvaluation previousEval = new DTODetailedFactorEvaluation(factorId, factorDescription,
                factorName, dtoMetricEvaluationList, "testType");
    ...
    public void shouldCreateFactorAlertWithThresholdTrespassed() throws IOException, ProjectNotFoundException,
        ...
        Factor factor = domainObjectsBuilder.buildFactor(project);
        float value = 0.2f; // threshold is 0.3f so the alert should be created at least for the threshold
                            // trespassed
    ...
    public void shouldCreateStrategicIndicatorAlertWithCategoryDowngrade() throws IOException, ProjectNotFoundException,
        ...
        when(siRepository.findByExternalIdAndProjectId(strategic_indicator.getExternalId(), project.getId()))
                .thenReturn(strategic_indicator);
        ...
        DTODetailedStrategicIndicatorEvaluation previousEval = new DTODetailedStrategicIndicatorEvaluation(
                dtoStrategicIndicatorEvaluation.getId(), dtoStrategicIndicatorEvaluation.getName(), dtoFactorList);
        previousEval.setValue(Pair.of(0.8f, "0.8"));
        List<DTODetailedStrategicIndicatorEvaluation> previousEvals = Arrays.asList(previousEval);
        when(qmaStrategicIndicators.HistoricalData(eq(strategic_indicator.getExternalId()), any(), any(),
                eq(project.getExternalId()), any())).thenReturn(previousEvals);

        // When
        alertsController.shouldCreateIndicatorAlert(strategic_indicator, 0.33f);

        // Then
        verify(siRepository, times(1)).findByExternalIdAndProjectId(strategic_indicator.getExternalId(),
                project.getId());
    ...
    @Test
    public void shouldCreateStrategicIndicatorAlertWithThresholdTrespassed()
            throws IOException, ProjectNotFoundException, MetricNotFoundException, QualityFactorNotFoundException,
    ...
    @Test
    public void shouldCreateThresholdNonTreatedAlert() throws IOException, MetricNotFoundException,
            QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
        ...
        when(metricRepository.findByExternalIdAndProjectId(currentMetricEval.getId(), projectId))
        ...
        when(alertRepository.findTopByProjectIdAndAffectedIdAndAffectedTypeAndTypeAndDateLessThanOrderByIdDesc(
                projectId, currentMetric.getExternalId(), "metric", AlertType.TRESPASSED_THRESHOLD,
                alertsController.getTodayStartOfDayInstant()))
        ...
        when(qmaMetrics.SingleHistoricalData(eq(currentMetric.getExternalId()), any(), any(),
                eq(project.getExternalId()), any())).thenReturn(previousEvals);
        ...
        verify(alertRepository, times(1))
                .findTopByProjectIdAndAffectedIdAndAffectedTypeAndTypeAndDateLessThanOrderByIdDesc(
                        projectId, currentMetric.getExternalId(), "metric", AlertType.TRESPASSED_THRESHOLD,
                        alertsController.getTodayStartOfDayInstant());
    ...
    @Test
    public void shouldNotCreateThresholdNonTreatedAlertBecauseOfDates() throws IOException, MetricNotFoundException,
            QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    @Test
    public void shouldNotCreateThresholdNonTreatedAlertBecauseOfImprovement() throws IOException,
            MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    @Test
    public void shouldCreateCategoryAlertNotTreated() throws IOException, MetricNotFoundException,
            QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
        ...
        // creating a previous evaluation that will have the same value, so it will be
        // in the same level and raise a check on non treated alerts
        DTOMetricEvaluation previousEval = domainObjectsBuilder.buildDTOMetric();
        List<DTOMetricEvaluation> previousEvals = Arrays.asList(previousEval);
        ...
        // creating previous alerts being the last upgrade alert older than the last
        // downgrade alert (so its a non treated alert as it hasn't improved)
        ...
        when(alertRepository.findTopByProjectIdAndAffectedIdAndAffectedTypeAndTypeAndDateLessThanOrderByIdDesc(
                metric.getProject().getId(), metric.getExternalId(), "metric", AlertType.CATEGORY_UPGRADE,
                alertsController.getTodayStartOfDayInstant()))
                .thenReturn(upgradeAlert);
        when(alertRepository.findTopByProjectIdAndAffectedIdAndAffectedTypeAndTypeAndDateLessThanOrderByIdDesc(
                metric.getProject().getId(), metric.getExternalId(), "metric", AlertType.CATEGORY_DOWNGRADE,
                alertsController.getTodayStartOfDayInstant()))
        ...
        verify(alertRepository, times(1))
                .findTopByProjectIdAndAffectedIdAndAffectedTypeAndTypeAndDateLessThanOrderByIdDesc(
                        projectId, metric.getExternalId(), "metric", AlertType.CATEGORY_DOWNGRADE,
                        alertsController.getTodayStartOfDayInstant());
        verify(alertRepository, times(1))
                .findTopByProjectIdAndAffectedIdAndAffectedTypeAndTypeAndDateLessThanOrderByIdDesc(
                        projectId, metric.getExternalId(), "metric", AlertType.CATEGORY_UPGRADE,
                        alertsController.getTodayStartOfDayInstant());
        verify(metricRepository, times(2)).findByExternalIdAndProjectId(metricEval.getId(), projectId);
        verify(metricCategoryRepository, times(1)).findAllByName(metric.getCategoryName());
        ArgumentCaptor<Alert> alertArgumentCaptor = ArgumentCaptor.forClass(Alert.class);
    ...
    @Test
    public void shouldCreateMetricPredictionAlertWithCategoryDowngrade()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
        ...
        // predicted evals (adding more than one that has value < threshold to check
        // that only one alert should checked and then stop
    ...
    @Test
    public void shouldCreateMetricPredictionAlertWithCategoryUpgrade()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    @Test
    public void shouldCreateMetricPredictionAlertWithThresholdTrespassed()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    @Test
    public void shouldCreateFactorPredictionAlertWithCategoryDowngrade()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
        ...
        // creating the current eval that will have a higher value, so it will be in a
        // different level thant the predicted and raise an alert
        ...
        List<Date> dates = Arrays.asList(java.sql.Date.valueOf(predictedEval.getDate()),
                java.sql.Date.valueOf(predictedEvalTwo.getDate()));

        // When
        alertsController.checkAlertsForFactorsPrediction(currentEval.getValue().getFirst(), factor.getExternalId(),
                values, dates, project.getExternalId(), "PROPHET");
    ...
    @Test
    public void shouldCreateFactorPredictionAlertWithCategoryUpgrade()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    @Test
    public void shouldCreateFactorPredictionAlertWithThresholdTrespassed()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    @Test
    public void shouldCreateIndicatorPredictionAlertWithCategoryDowngrade()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
        ...
        // When
        alertsController.checkAlertsForIndicatorsPrediction(currentEval.getValue().getFirst(), si.getExternalId(),
                values, dates, project.getExternalId(), "PROPHET");
    ...
    @Test
    public void shouldCreateIndicatorPredictionAlertWithCategoryUpgrade()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    @Test
    public void shouldCreateIndicatorPredictionAlertWithThresholdTrespassed()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
    ...
    @Test
    public void shouldNotCreateMetricPredictionAlertBecauseTodayExactAlertHasBeenCreated()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
        ...
        when(alertRepository
                .findAlertByProjectIdAndAffectedIdAndAffectedTypeAndTypeAndPredictionTechniqueAndPredictionDateAndDateGreaterThanEqualAndDateLessThan(
                        eq(projectId), eq(alert.getAffectedId()), eq("metric"), eq(alert.getType()), eq("PROPHET"),
                        eq(alert.getPredictionDate()), eq(startDate), any()))
                .thenReturn(alert);
    ...
    @Test
    public void shouldCreateMetricPredictionAlertBecauseTodayExactAlertHasBeenCreatedButItIsDifferent()
            throws MetricNotFoundException, QualityFactorNotFoundException, StrategicIndicatorNotFoundException {
        // Given
        Project project = domainObjectsBuilder.buildProject();
        Long projectId = project.getId();
